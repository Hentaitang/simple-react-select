...
const component = memo(
  ({
    ...
    const { width, selectColor, selectBorderRadius, selectBorderColor, activeBorderColor } = selectCustomStyle;
    const { maxHeight, optionsBorderColor, optionsBorderRadius, optionHoverBgColor, optionActiveBgColor, optionActiveColor } = optionsCustomStyle;
        ...
        listHtml = stateSelectList[0].map((child, index) => {
          return (
            <li
              style={{
                backgroundColor:
                  stateSelect[0] === child.props.value
                    ? optionActiveBgColor
                      ? optionActiveBgColor
                      : '#e8f0fb'
                    : stateCurrentItemIndex[0] === index
                    ? optionHoverBgColor
                      ? optionHoverBgColor
                      : '#f5f7fa'
                    : '',
                color: child.props.disabled ? '#d9d9d9' : stateSelect[0] === child.props.value ? (optionActiveColor ? optionActiveColor : '#409eff') : child.props.color ? child.props.color : '',
    ...
    return (
      <div className={STYLES['selectWrapper']} ref={selectRef} style={{ width }}>
        <div
          className={`${STYLES['inputWrapper']}`}
          style={{
            paddingRight: stateInputValue[0] && isClearable ? '60px' : '',
            color: selectColor,
            borderRadius: selectBorderRadius,
            borderColor: stateIsfocus[0] ? (activeBorderColor ? activeBorderColor : '#409eff') : selectBorderColor ? selectBorderColor : '#dcdfe6',
          }}
          onClick={() => {
            stateIsfocus[1](true);
        ...
        >
          ...
          {!stateInputValue[0] && stateSelect[0] ? (
          ...
          <svg width="16px" height="16px" className={`${STYLES['dropdownIcon']} ${stateIsShowList[0] ? STYLES['active'] : ''}`} onClick={() => {}} viewBox="0 0 1024 1024">
            <path
        ...
        {stateIsShowList[0] ? (
          <div
            className={STYLES['selectListWrapper']}
            ref={listWrapRef}
            style={{ maxHeight, borderColor: optionsBorderColor ? optionsBorderColor : '', borderRadius: optionsBorderRadius ? optionsBorderRadius : '' }}
          >
